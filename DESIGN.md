...
- Evidence reviewed: `docs/index.html`, `docs/assets/dashboard.js`, `docs/assets/styles.css`, `docs/data/dashboard.json`, `momentum_factor_lab/dashboard.py`, prior repo memory that CSS output and embedded `CSS_CONTENT` must stay synchronized; public/local connected dashboard CSS from `quant-dashboard`, `best-factor`, `etf-tracking`, `dram-price`, `valuation`, and `port`.
...
- Avoid: low-contrast near-black layering, overly bright blue-gray surfaces that drift away from the connected dashboards, decorative glow that competes with numbers, implying browser scenarios are saved trading instructions.
...
## Visual language
- Color: connected-dashboard family palette: shared navy/blue/cyan anchors (`#132033`, `#2457d6`, `#44b3ff`) on near-black/charcoal dark surfaces (`#080a0f`, `#12161e`, `#171d29`). Keep the dark concept close to `port`, but use brighter foregrounds and borders for readability. Green/amber/rose are semantic only.
...
- Design-token constraints: keep CSS variables and final dark readability override in sync between source template and generated CSS; do not introduce a one-off palette that visually separates this dashboard from connected Pages projects.
